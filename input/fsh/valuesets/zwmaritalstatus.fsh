ValueSet: ZimMaritalStatusVS
Id: ZimMaritalStatus
Title: "MaritalStatus Value Set"
Description: "Administrative Marital Status"
* ^experimental = false
* http://terminology.hl7.org/CodeSystem/v3-MaritalStatus#W "Widowed"
* http://terminology.hl7.org/CodeSystem/v3-MaritalStatus#D "Divorced"
* http://terminology.hl7.org/CodeSystem/v3-MaritalStatus#M "Married"
* http://terminology.hl7.org/CodeSystem/v3-MaritalStatus#U "Single"
* http://terminology.hl7.org/CodeSystem/v3-MaritalStatus#L "Separated"
* http://terminology.hl7.org/CodeSystem/v3-MaritalStatus#C "Cohabitation"
